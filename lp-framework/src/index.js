...
function new_script(account) {
  return new Promise(function(resolve, reject){
    let script = document.createElement('script');
    script.src = "https://d2tjvvl6yqo45i.cloudfront.net/analytics/" + account + "/script.js";
...
export default class LpFramework extends ReactQueryParams {
  ...
  componentDidMount(){
    ...
    new_script(this.state.leadData.account_id).then(() => {
  ...
  updateState(countryCode, languageJson, phonePrefix, lang) {
    this.setState({
      ...
      leadData: {...this.state.leadData,
        phone_country_prefix: (phonePrefix.indexOf("+") > -1 ? "+" : "") + (phonePrefix ? phonePrefix.toString() : ""),
        language: lang
      }
    });
  }
  ...
  handleLeadStep = (data) => {
    ...
    LeadHandler.sendLead('/lead_first_step', this.state.leadData).then(res => {
      ...
      this.setState({
        ...
        if (window.sbidTracking) {
          window.sbidTracking.settings.params.email = this.state.leadData.email;
          window.sbidTracking.settings.params.fname = this.state.leadData.first_name;
          window.sbidTracking.settings.params.lname = this.state.leadData.last_name;
          window.sbidTracking.track({e: 'lead_next1'});
        }
      })
    });
  ...
  validateParams = (params) => {
    ...
    Object.keys(params).forEach(p => {
      ...
      if (p === "email") {
        let trimmedEmail = params[p].trim();
        ...
        let trimmedLastName = params[p].trim();

        if (trimmedLastName.length === 0) {
          errors.push("Last name is empty");
        }
        if (!/^([^0-9]*)$/.test(trimmedLastName)) {
          errors.push("Please enter last name without digits")
        }
      }
      else if (p === "first_name") {
        let trimmedFirstName = params[p].trim();

        if (trimmedFirstName.length === 0) {
          errors.push("First name is empty");
        }
        if (!/^([^0-9]*)$/.test(trimmedFirstName)) {
  ...
  handleSubmit = (data) => {
    ...
    if(window.sbidTracking){
      window.sbidTracking.settings.params.phone_number = data.phone_number.trim();
    }
    ...
    //   phoneNumber = {
    //     phone_number: this.state.leadData.phone_country_prefix + data.phone_number.trim()
    //   }
    // }
    //
    // console.log(data);
    ...
    return new Promise((resolve, reject) => {
      ...
      let trimmedFirstName = this.state.leadData["first_name"].trim();
      let trimmedLastName = this.state.leadData["last_name"].trim();
      let trimmedEmail= this.state.leadData["email"].trim();
      ...
      this.setState({
        leadData:{
          ...
          phone_country_prefix: !data.phone_country_prefix ? this.state.leadData.phone_country_prefix: data.phone_country_prefix,
          validation: (trimmedEmail === "test" || trimmedLastName === "test" || window.location.host.includes("localhost")) ? 3 : this.state.validation
        }

      }, () => {

        resolve(LeadHandler.sendLead('/leads', this.state.leadData).then(res => {
